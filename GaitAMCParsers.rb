...
def femur_parser(rotation_data)
  ...
  rz = rotation_data[1].to_f
  ry = rotation_data[2].to_f - 20.0
...
def humerus_parser(rotation_data)
  rx = rotation_data[0].to_f
  rz = rotation_data[1].to_f
  ry = rotation_data[2].to_f
  {roll:rx, pitch: ry, yaw: rz}
end

def spine_parser(rotation_data)
  rx = rotation_data[0].to_f
  rz = rotation_data[1].to_f
  ry = rotation_data[2].to_f
  {roll:rx, pitch: ry, yaw: rz}
...
def root_parser(rotation_data)
  rx = rotation_data[0].to_f #+ 180.0
  rz = rotation_data[1].to_f
  ry = rotation_data[2].to_f# + 180.0
  {roll:rx, pitch: ry, yaw: rz}
end
